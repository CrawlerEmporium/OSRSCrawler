...
async def on_guild_join(guild):
    tracker = await bot.fetch_channel(TRACKER)
    bots = sum(1 for m in guild.members if m.bot)
    members = len(guild.members)
    ratio = bots / members
    await tracker.send(
        f"I was invited to **{guild}** ({guild.id}).\nIt's owner is **{guild.owner}** ({guild.owner.mention} {guild.owner.id}).\nI now am in #{len(bot.guilds)} servers.")
...
async def on_guild_remove(guild):
    tracker = await bot.fetch_channel(TRACKER)
    await tracker.send(
        f"I was removed from **{guild}** ({guild.id}).\nIt's owner is **{guild.owner}** ({guild.owner.mention} {guild.owner.id}).\nI now am in #{len(bot.guilds)} servers.")
    await bot.change_presence(activity=discord.Game(f"with {len(bot.guilds)} servers | !help | {version}"),
                              afk=True)
